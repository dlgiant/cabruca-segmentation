...
class COCOConverter:
    ...
    def save_coco_dataset(self, output_path: str) -> None:
        ...
        logger.info(f"Dataset contains {len(self.coco_dataset['images'])} images and "
    ...
    def validate_coco_dataset(self) -> Dict[str, Any]:
        ...
        for cat in self.coco_dataset["categories"]:
            cat_id = cat["id"]
            cat_name = cat["name"]
            ann_count = sum(1 for ann in self.coco_dataset["annotations"] 
                           if ann["category_id"] == cat_id)
            stats["annotations_per_category"][cat_name] = ann_count
            
            # Count images with this category
            image_ids = set(ann["image_id"] for ann in self.coco_dataset["annotations"] 
                           if ann["category_id"] == cat_id)
        ...
        annotated_images = set(ann["image_id"] for ann in self.coco_dataset["annotations"])
