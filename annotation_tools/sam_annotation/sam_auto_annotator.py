...
class SAMAutoAnnotator:
    ...
    def classify_mask_by_properties(self, mask: Dict, image: np.ndarray) -> int:
        ...
        if len(masked_region) > 0:
            ...
            is_vegetation = (
                mean_color[1] > mean_color[0] and mean_color[1] > mean_color[2]
            )
        else:
            is_vegetation = False
    ...
    def masks_to_coco_annotations(
        self, masks: List[Dict], image_id: str, image_shape: Tuple[int, int]
    ) -> List[Dict]:
    ...
    def annotate_image(
        self, image_path: str, output_dir: str, interactive: bool = False
    ) -> Dict:
        """
    ...
    def batch_annotate(
        ...
        image_dir: str,
        output_dir: str,
        image_extensions: List[str] = [".jpg", ".png", ".tif"],
    ) -> List[Dict]:
    ...
    def visualize_annotations(
        self, image_path: str, annotations: List[Dict], save_path: Optional[str] = None
    ):
        ...
        for ann in annotations:
            ...
            x, y, w, h = ann["bbox"]
            cv2.rectangle(
                image_rgb, (int(x), int(y)), (int(x + w), int(y + h)), color, 2
            )
