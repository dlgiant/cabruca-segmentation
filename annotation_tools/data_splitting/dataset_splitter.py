...
class DatasetSplitter:
    ...
    def __init__(self, coco_json_path: str, seed: int = 42):
        ...
        logger.info(
            f"Loaded dataset with {len(self.images)} images and {len(self.annotations)} annotations"
        )
    ...
    def analyze_class_distribution(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "avg_annotations_per_image": (
                total_annotations / len(self.images) if self.images else 0
            ),
        }
    ...
    def stratified_split(
        ...
        logger.info(
            f"Split: Train={len(train_images)}, Val={len(val_images)}, Test={len(test_images)}"
        )
    ...
    def balance_classes(
        ...
        if strategy == "oversample":
            ...
            logger.warning(
                "Augmentation strategy not fully implemented, using oversampling"
            )
            sampler = RandomOverSampler(random_state=self.seed)
            X_resampled, y_resampled = sampler.fit_resample(X, y)
        ...
        logger.info(
            f"Balanced dataset from {len(images)} to {len(balanced_images)} images"
        )
    ...
    def create_split_datasets(
        ...
        for split_name, split_images in splits.items():
            ...
            split_annotations = [
                ann for ann in self.annotations if ann["image_id"] in image_ids
            ]
    ...
    def visualize_class_distribution(self, save_path: Optional[str] = None):
        ...
        colors = [
            ...
            "#00FF00",
            "#FF0000",
        ]  # Gray, Green, Red for bg, cacao, shade
...
"""
    ...
    def create_balanced_subset(
        self, max_samples_per_class: int = 100, output_path: Optional[str] = None
    ) -> Dict:
        """
