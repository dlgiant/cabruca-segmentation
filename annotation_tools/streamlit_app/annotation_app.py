...
def main():
    ...
    st.markdown("### Interactive annotation interface for cacao and shade tree detection")
    ...
    with st.sidebar:
        ...
        drawing_mode = st.selectbox(
            "Drawing Mode",
            options=["polygon", "rectangle", "circle", "point"],
            index=0
        )
        ...
        if st.session_state.current_image in st.session_state.annotations:
            current_annotations = st.session_state.annotations[st.session_state.current_image]
            
            stats_data = []
            for class_id, class_info in CLASSES.items():
                count = sum(1 for ann in current_annotations if ann["class_id"] == class_id)
                stats_data.append({
        ...
        with col1:
            if st.button("Save Annotations", type="primary"):
                if st.session_state.current_image and st.session_state.current_image in st.session_state.annotations:
    ...
    if st.session_state.image_list:
        ...
        with col2:
            ...
            if current_annotations:
                for i, ann in enumerate(current_annotations):
                    ...
                    with st.expander(f"{class_info['name']} - {ann.get('shape_type', 'polygon')} #{i+1}"):
                        st.write(f"Class: {class_info['name']}")
                        st.write(f"Type: {ann.get('shape_type', 'polygon')}")
                        st.write(f"Points: {len(ann['points'])}")
                        
                        if st.button(f"Delete", key=f"delete_{i}"):
                            st.session_state.annotations[st.session_state.current_image].pop(i)
                            st.rerun()
            else:
        ...
        with col2:
            st.markdown(f"<center>Image {st.session_state.current_index + 1} of {len(st.session_state.image_list)}</center>", unsafe_allow_html=True)
        
        with col3:
            if st.button("Next ➡️", disabled=st.session_state.current_index == len(st.session_state.image_list) - 1):
                st.session_state.current_index += 1
                st.rerun()
