...
class AnnotationPipeline:
    ...
    def run_sam_annotation(self, image_dir: str, output_dir: Optional[str] = None) -> str:
        """
        ...
        logger.info(f"Completed SAM annotation. Generated {len(annotations)} annotation files")
        
        return str(output_dir)
    
    def convert_to_coco(self, annotation_dirs: list, output_path: Optional[str] = None,
                       format_types: Optional[Dict[str, str]] = None) -> str:
        """
        ...
        for ann_dir in annotation_dirs:
            ...
            logger.info(f"Converting annotations from {ann_dir} (format: {format_type})")
    ...
    def split_dataset(self, coco_path: str, output_dir: Optional[str] = None,
                     train_ratio: float = 0.7, val_ratio: float = 0.15,
                     test_ratio: float = 0.15, balance_strategy: Optional[str] = None) -> Dict[str, str]:
        """
    ...
    def run_full_pipeline(self, image_dir: str, balance_strategy: str = "mixed") -> Dict:
        """
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Annotation and Data Preparation Pipeline")
    parser.add_argument("--image-dir", type=str, help="Directory containing images to annotate")
    parser.add_argument("--annotation-dirs", nargs="+", help="Directories with existing annotations")
    parser.add_argument("--output-dir", type=str, help="Output directory for processed data")
    parser.add_argument("--balance-strategy", type=str, default="mixed",
    ...
    parser.add_argument("--convert-only", action="store_true", help="Only convert annotations to COCO")
    parser.add_argument("--split-only", action="store_true", help="Only split existing COCO dataset")
    ...
    if args.run_sam and args.image_dir:
        ...
        outputs = pipeline.split_dataset(
            ...
            balance_strategy=args.balance_strategy if args.balance_strategy != "none" else None
        )
        ...
        print("  python annotation_pipeline.py --convert-only --annotation-dirs data/annotations/sam data/annotations/labelme")
        print("\n  # Split existing COCO dataset:")
        print("  python annotation_pipeline.py --split-only --coco-path data/processed/annotations_coco.json")
