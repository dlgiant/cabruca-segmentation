...
Examples:
  ...
  python infer.py --model checkpoint.pth --images data/ --device mps
    ...
    if not args.quiet:
        ...
        cacao_total = sum(sum(1 for t in r.trees if t.species == 'cacao') for r in results)
        shade_total = sum(sum(1 for t in r.trees if t.species == 'shade') for r in results)
