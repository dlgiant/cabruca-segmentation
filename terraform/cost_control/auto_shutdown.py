...
def lambda_handler(event, context):
    ...
    results = {
        ...
        "resources": {"lambda_functions": [], "ec2_instances": [], "rds_instances": []},
    }
...
def shutdown_resources(environment, tags, results):
    ...
    try:
        ...
        for func in functions["Functions"]:
            ...
            if (
                environment in func_name
                and func_tags.get("AutoShutdown") == "true"
                and "critical" not in func_tags.get("Type", "").lower()
            ):
                ...
                results["resources"]["lambda_functions"].append(
                    {"name": func_name, "status": "shutdown", "concurrency": 0}
                )
    ...
    try:
        ...
        for db in response["DBInstances"]:
            ...
            if (
                db_tags.get("Environment") == environment
                and db_tags.get("AutoShutdown") == "true"
                ...
                results["resources"]["rds_instances"].append(
                    {"id": db_id, "engine": db["Engine"], "status": "stopping"}
                )
        ...
        for func in functions["Functions"]:
            ...
            try:
                ...
                if concurrency.get("ReservedConcurrentExecutions") == 0:
                    ...
                    logger.info(
                        f"Started Lambda function: {func_name} with concurrency: {new_concurrency}"
                    )
        ...
        for db in response["DBInstances"]:
            ...
            if db["DBInstanceStatus"] == "stopped":
                ...
                if (
                    ...
                    results["resources"]["rds_instances"].append(
                        {"id": db_id, "engine": db["Engine"], "status": "starting"}
                    )
