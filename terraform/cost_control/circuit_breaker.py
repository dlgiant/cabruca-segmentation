...
def lambda_handler(event, context):
    ...
    results = {
        ...
        "limits": {"monthly": monthly_limit, "daily": daily_limit},
        "costs": {},
        "actions": [],
    }
...
def get_current_costs():
    ...
    try:
        ...
        costs["projected_monthly_total"] = costs.get("month_to_date", 0) + costs.get(
            "monthly_forecast", 0
        )
...
def should_trigger_circuit_breaker(costs, monthly_limit, daily_limit):
    ...
    if costs.get("projected_monthly_total", 0) > monthly_limit:
        logger.warning(
            f"Projected monthly total ${costs.get('projected_monthly_total', 0):.2f} exceeds limit ${monthly_limit}"
        )
        return True
    ...
    if costs.get("last_24_hours", 0) > (daily_limit * 3):
        logger.warning(
            f"Daily costs ${costs.get('last_24_hours', 0):.2f} exceed 3x normal rate ${daily_limit * 3:.2f}"
        )
        return True
    ...
    if lambda_costs > (
        monthly_limit * 0.5
    ):  # Lambda shouldn't be more than 50% of budget
    ...
    for func_name, concurrency in function_configs:
        if func_name:
            try:
                ...
                logger.info(
                    f"Re-enabled function: {func_name} with concurrency: {concurrency}"
                )
...
def disable_event_rules():
    ...
    for rule_name in rule_patterns:
        try:
            ...
            actions.append(
                {"action": "disable_rule", "rule": rule_name, "status": "success"}
            )
...
def enable_event_rules():
    ...
    for rule_name in rule_patterns:
        try:
            ...
            actions.append(
                {"action": "enable_rule", "rule": rule_name, "status": "success"}
            )
    ...
    try:
        ...
        if metrics:
            cloudwatch_client.put_metric_data(
                Namespace="CostControl/CircuitBreaker", MetricData=metrics
            )
