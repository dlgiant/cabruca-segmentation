...
class EventBridgeCostEstimator:
    ...
    def calculate_total_cost(self, days: int = 30) -> Dict[str, any]:
        ...
        return {
            ...
            "summary": {
                ...
                "needs_alert": total_cost
                >= (self.MONTHLY_BUDGET * self.ALERT_THRESHOLD),
            },
        }
    ...
    def simulate_burst_scenario(
        self, burst_multiplier: float = 3.0, burst_duration_hours: int = 4
    ) -> Dict[str, any]:
        ...
        for agent_type in self.agent_patterns:
            ...
            burst_events[agent_type] = {
                ...
                "increase_percent": ((total_day_events / pattern["events_per_day"]) - 1)
                * 100,
            }
        ...
        return {
            ...
            "cost_increase_percent": (
                (burst_cost / normal_day["summary"]["total_cost"]) - 1
            )
            * 100,
            "burst_events": burst_events,
            "monthly_impact": {
                "with_1_burst_day": (normal_day["summary"]["total_cost"] * 29)
                + burst_cost,
                "with_5_burst_days": (normal_day["summary"]["total_cost"] * 25)
                + (burst_cost * 5),
                "with_10_burst_days": (normal_day["summary"]["total_cost"] * 20)
                + (burst_cost * 10),
            },
        }
    ...
    def generate_report(self) -> str:
        ...
        if not summary["is_within_budget"]:
            ...
            for optimization in opt["optimizations"]:
                ...
                report.append(
                    f"  Current: {optimization['current_daily']:,} events/day"
                )
                report.append(
                    f"  Suggested: {optimization['suggested_daily']:,} events/day"
                )
        else:
            report.append("\n✅ Within budget")
        ...
        report.append("-" * 40)
        report.append(
            f"Scenario: {burst['burst_multiplier']}x traffic for {burst['burst_duration_hours']} hours"
        )
        report.append(f"Normal Daily Cost: ${burst['normal_daily_cost']:.2f}")
        report.append(f"Burst Daily Cost: ${burst['burst_daily_cost']:.2f}")
        report.append(
            f"Cost Increase: ${burst['cost_increase']:.2f} ({burst['cost_increase_percent']:.1f}%)"
        )
...
def main():
    ...
    parser.add_argument(
        "--days", type=int, default=30, help="Number of days to calculate (default: 30)"
    )
    parser.add_argument(
        "--budget",
        type=float,
    ...
    parser.add_argument(
        "--optimize", action="store_true", help="Show optimization suggestions"
    )
    parser.add_argument(
        "--burst", action="store_true", help="Include burst scenario analysis"
    )
