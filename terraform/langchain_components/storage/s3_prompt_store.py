...
class S3PromptStore:
    ...
    def export_prompts(self, output_format: str = "json") -> str:
        ...
        try:
            ...
            for agent_type in agent_types:
                ...
                agent_prompts = [
                    p for p in prompts_list if p["agent_type"] == agent_type
                ]

                for prompt_info in agent_prompts:
                    prompt_content = self.get_prompt(
                        agent_type, prompt_info["prompt_name"]
                    )
                    if prompt_content:
                        all_prompts[agent_type][
    ...
    def _is_cache_valid(self) -> bool:
        ...
        time_since_refresh = (
            datetime.utcnow() - self.last_cache_refresh
        ).total_seconds()
...
class PromptVersionManager:
    ...
    def get_prompt_versions(
        self, agent_type: str, prompt_name: str
    ) -> List[Dict[str, Any]]:
        """
    ...
    def restore_version(self, agent_type: str, prompt_name: str, version: str) -> bool:
        ...
        try:
            ...
            version_key = (
                f"{self.prefix}{agent_type}/versions/{prompt_name}_{version}.txt"
            )
            ...
            logger.info(
                f"Restored prompt {prompt_name} for {agent_type} from version {version}"
            )
            return True
    ...
    def compare_versions(
        self, agent_type: str, prompt_name: str, version1: str, version2: str
    ) -> Dict[str, Any]:
        """
...
def initialize_prompt_store(bucket_name: str) -> S3PromptStore:
    ...
    logger.info(
        f"Initialized prompt store with {successful}/{len(results)} default prompts"
    )
