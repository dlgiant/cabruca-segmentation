...
class LLMConfig:
    ...
    def estimated_cost_per_call(self) -> float:
        ...
        ) * self.model_type.input_cost_per_million
        output_cost = (
            avg_output_tokens / 1_000_000
        ) * self.model_type.output_cost_per_million
    ...
    def on_llm_end(self, response: LLMResult, **kwargs) -> None:
        ...
        try:
            ...
            if response.llm_output:
                ...
                # Log metrics
                logger.info(
                    f"LLM Call - Agent: {self.agent_name}, Model: {self.model_type.name}, "
...
class LLMProvider:
    ...
    def get_llm(
        self, model_type: Optional[ModelType] = None, config: Optional[LLMConfig] = None
    ) -> ChatAnthropic:
...
class FallbackLLMChain:
    ...
    def __init__(self, agent_name: str):
        ...
        self.model_hierarchy = [
            (
                ModelType.HAIKU,
                LLMConfig(ModelType.HAIKU, temperature=0.3, max_tokens=1000),
            ),
            (
                ModelType.SONNET,
                LLMConfig(ModelType.SONNET, temperature=0.3, max_tokens=2000),
            ),
            (
                ModelType.OPUS,
                LLMConfig(ModelType.OPUS, temperature=0.2, max_tokens=4000),
            ),
        ]
    ...
    def invoke(self, prompt: str, require_complex_reasoning: bool = False) -> str:
        ...
        for i in range(start_index, len(self.model_hierarchy)):
            ...
            try:
                ...
                # Log successful call
                logger.info(
                    f"Successfully used {model_type.name} for {self.agent_name}"
                )
...
class ModelSelector:
    ...
    def select_model(task_type: str, estimated_complexity: str = "low") -> ModelType:
        ...
        task_model_map = {
            ...
            "code_generation": (
                ModelType.SONNET if estimated_complexity != "low" else ModelType.HAIKU
            ),
            "test_generation": (
                ModelType.SONNET if estimated_complexity != "low" else ModelType.HAIKU
            ),
            ...
            "architecture_design": (
                ModelType.OPUS if estimated_complexity == "high" else ModelType.SONNET
            ),
            "complex_debugging": (
                ModelType.OPUS if estimated_complexity == "high" else ModelType.SONNET
            ),
...
class PromptOptimizer:
    ...
    def create_concise_prompt(template: str, variables: Dict[str, Any]) -> str:
        ...
        filtered_lines = [
            line
            for line in lines
            if not any(marker in line for marker in ["None", "N/A", "[]", "{}"])
        ]
