        ...
        try:
            ...
            if datapoints:
                ...
                result = {
                    ...
                    'statistics': {
                        'avg': sum(p.get('Average', 0) for p in sorted_points) / len(sorted_points) if sorted_points else 0,
                        'max': max(p.get('Maximum', 0) for p in sorted_points) if sorted_points else 0,
                        'min': min(p.get('Minimum', float('inf')) for p in sorted_points) if sorted_points else 0
                    }
                }
                
                return json.dumps(result, default=str)
            else:
                return json.dumps({'error': 'No datapoints found', 'metric': metric_name})
...
class CodeBuildTool(BaseTool):
    ...
    def _run(
        ...
        try:
            ...
            if action == 'start':
                return self._start_build(project_name, params.get('environment_variables', {}))
            elif action == 'status':
...
class CloudWatchLogsTool(BaseTool):
    ...
    def _run(
        ...
        try:
            ...
            query_string = params.get('query', 'fields @timestamp, @message | sort @timestamp desc | limit 20')
            ...
            while wait_time < max_wait:
                ...
                if results_response['status'] == 'Complete':
                    return json.dumps({
                        'status': 'complete',
                        'results': results_response['results'][:50],  # Limit to 50 results
                        'statistics': results_response.get('statistics', {}),
                        'records_matched': results_response['statistics']['recordsMatched'] if 'statistics' in results_response else 0
                    })
    ...
    def _run(
        ...
        try:
            ...
            if action == 'list_services':
                ...
                return self._update_service(cluster, params.get('service'), params.get('desired_count', 1))
    ...
    def _run_task(self, cluster: str, task_definition: str) -> str:
        ...
        try:
            ...
            if response['tasks']:
                ...
                return json.dumps({'error': 'Failed to run task', 'failures': response.get('failures', [])})
