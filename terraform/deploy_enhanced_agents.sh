...
echo "========================================="
echo "🚀 Deploying Enhanced Multi-Agent System"
echo "========================================="

# Save current simple versions
cp manager_agent/lambda_function.py manager_agent/lambda_function_simple.py
cp engineer_agent/lambda_function.py engineer_agent/lambda_function_simple.py  
cp qa_agent/lambda_function.py qa_agent/lambda_function_simple.py

# Create enhanced versions
cat > manager_agent/lambda_function_enhanced.py << 'MANAGER_EOF'
import json
import os
import boto3
...
class ManagerAgent:
    ...
    def __init__(self):
        ...
        self.state_table = os.environ.get('DYNAMODB_STATE_TABLE', f'cabruca-mvp-{self.environment}-agent-state')
        self.tasks_table = os.environ.get('DYNAMODB_TASKS_TABLE', f'cabruca-mvp-{self.environment}-agent-tasks')
    ...
    def orchestrate_fixes(self, issues: List[Dict], repo_info: Dict) -> Dict:
        ...
        for issue in issues:
            workflow_id = f"fix-{issue['type']}-{datetime.now().strftime('%Y%m%d%H%M%S')}"
            
            # Determine which agents to involve
            agents_needed = []
            
            if issue['type'] in ['build_failure', 'quality_issue', 'security_vulnerability']:
    ...
    def dispatch_to_agent(self, agent_type: str, workflow_id: str, issue: Dict, repo_info: Dict):
...
# Copy enhanced versions to actual files
cp manager_agent/lambda_function_enhanced.py manager_agent/lambda_function.py
...
terraform apply -var-file="mvp.tfvars" \
    -var="alert_email=alerts@theobroma.digital" \
    -target=aws_lambda_function.agents[\"manager\"] \
    -auto-approve

echo "✅ Enhanced manager agent deployed\!"
echo ""
echo "Test with:"
echo "./test_pipeline_orchestration.sh"
