...
def create_sample_predictions():
    ...
    predictions = {
        "instances": {
            ...
            "scores": np.array(
                [0.95, 0.92, 0.88, 0.91, 0.89, 0.87, 0.93, 0.90, 0.94, 0.86, 0.65]
            ),
    ...
    for box, label in zip(
        predictions["instances"]["boxes"], predictions["instances"]["labels"]
    ):
    ...
    semantic_map[semantic_map == 0] = np.random.choice(
        [0, 3, 4], size=(semantic_map == 0).sum(), p=[0.5, 0.3, 0.2]
    )
...
def create_sample_ground_truth():
    ...
    for box, label in zip(
        ground_truth["instances"]["boxes"], ground_truth["instances"]["labels"]
    ):
        x1, y1, x2, y2 = box.astype(int)
        x1, x2 = x1 * 2, x2 * 2
        y1, y2 = y1 * 2, y2 * 2
...
def test_tree_count_accuracy():
    ...
    print(
        f"  Species classification accuracy: {metrics['species_classification_accuracy']:.2%}"
    )
    print(f"\nPer-species metrics:")
    print(
        f"  Cacao - Precision: {metrics['cacao_precision']:.3f}, Recall: {metrics['cacao_recall']:.3f}, F1: {metrics['cacao_f1']:.3f}"
    )
    print(
        f"  Shade - Precision: {metrics['shade_precision']:.3f}, Recall: {metrics['shade_recall']:.3f}, F1: {metrics['shade_f1']:.3f}"
    )
...
def test_spacing_uniformity():
    ...
    print(
        f"  Avg nearest neighbor distance: {metrics['avg_nearest_neighbor_distance']:.2f} m"
    )
    print(
        f"  Std nearest neighbor distance: {metrics['std_nearest_neighbor_distance']:.2f} m"
    )
...
def test_shade_distribution():
    ...
    print(
        f"  Avg cacao to shade distance: {metrics.get('avg_cacao_to_shade_distance', 0):.2f} m"
    )
    print(
        f"  Shade distribution score: {metrics.get('shade_distribution_score', 0):.3f}"
    )
