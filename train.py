...
def main():
    ...
    agentops.init(
        auto_start_session=False, tags=["cabruca", "training", "segmentation"]
    )
...
Examples:
  ...
  python train.py --config configs/training_config.yaml --lr 0.001 --batch-size 8 --epochs 50
    ...
    # Experiment tracking
    parser.add_argument(
        "--use-mlflow", action="store_true", help="Enable MLflow tracking"
    )
    parser.add_argument(
    ...
    # Validation
    parser.add_argument(
        "--val-frequency", type=int, default=None, help="Validation frequency (epochs)"
    )
    parser.add_argument(
        "--save-frequency",
        type=int,
    ...
    if not args.quiet:
        ...
        print(
            f"  Model: {config['model'].get('num_instance_classes', 3)} instance classes, "
        ...
        print(
            f"  Optimizer: {config['optimizer']['type']}, lr={config['optimizer']['lr']}"
        )
        print(f"  Training: {config['training']['num_epochs']} epochs")
        print(f"  Device: {config.get('device', 'auto')}")

        effective_batch = config["data"]["batch_size"] * config["training"].get(
    ...
    experiment_name = (
        ...
        or f"cabruca_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    tracer = agentops.start_trace(
        trace_name=experiment_name, tags=["training", "model-training"]
    )
