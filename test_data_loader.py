...
def test_simple_loader():
    ...
    for geojson_path in geojson_files:
        if geojson_path.exists():
            ...
            for i, aoi in enumerate(aois):
                ...
                print(
                    f"    - Bounds: ({aoi.bounds[0]:.4f}, {aoi.bounds[1]:.4f}) to ({aoi.bounds[2]:.4f}, {aoi.bounds[3]:.4f})"
                )
                ...
                if i == 0:
                    ...
                    print(
                        f"    - Planet format: {planet_format['type']} with {len(planet_format['coordinates'][0])} points"
                    )
        else:
    ...
    print(
        f"  Conversion error: {abs(back_lon - test_lon):.8f}, {abs(back_lat - test_lat):.8f}"
    )
    ...
    print(
        f"  Pixel resolution: {meta_dict['pixel_resolution_m'][0]:.2f}m x {meta_dict['pixel_resolution_m'][1]:.2f}m"
    )
...
def test_full_loader():
    ...
    try:
        ...
        print(
            f"  NDVI calculated: shape={ndvi.shape}, range=[{ndvi.min():.3f}, {ndvi.max():.3f}]"
        )

        # Calculate EVI
        evi = processor.calculate_evi(red, nir, blue)
        print(
            f"  EVI calculated: shape={evi.shape}, range=[{evi.min():.3f}, {evi.max():.3f}]"
        )

        # Test normalization
        normalized = processor.normalize_bands(ndvi, method="percentile")
        print(
            f"  Normalized NDVI: range=[{normalized.min():.3f}, {normalized.max():.3f}]"
        )
            ...
            if aois:
                ...
                if results:
                    ...
                    for i, item in enumerate(results[:3]):
                        ...
                        print(
                            f"    {i+1}. ID: {item_id[:20]}... | Date: {acquired} | Cloud: {cloud_cover:.1%}"
                        )
...
def main():
    ...
    print(
        f"  {'✓' if FULL_LOADER_AVAILABLE else '✗'} Full loader: {'Available' if FULL_LOADER_AVAILABLE else 'Not available (install dependencies)'}"
    )
    ...
    print(
        f"  {'✓' if planet_available else '✗'} Planet Labs: {'API key configured' if planet_available else 'No API key'}"
    )
