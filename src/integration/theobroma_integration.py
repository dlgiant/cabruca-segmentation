...
from inference.batch_inference import BatchInferenceEngine, InferenceResult, TreeInstance
...
class TheobromaIntegration:
    ...
    def compare_with_ml_detection(self, image_path: str, 
        ...
        if 'trees' in self.plantation_data:
            ...
            for feature in self.plantation_data['features']:
                tree = {
                    ...
                    'crown_diameter_m': feature.get('properties', {}).get('crown_diameter', 0)
                }
                trees.append(tree)
        
        return trees
    
    def _match_trees(self, ml_trees: List[TreeInstance], 
                    plantation_trees: List[Dict],
                    threshold: float) -> List[TreeComparison]:
        """
        ...
        if not ml_trees or not plantation_trees:
            ...
            for ml_tree in ml_trees:
                comparisons.append(TreeComparison(
                    plantation_id=-1,
        ...
        plant_positions = np.array([tree.get('coordinates', [0, 0])[:2] 
                                   for tree in plantation_trees])
    ...
    def _calculate_comparison_stats(self, comparisons: List[TreeComparison]) -> Dict:
        ...
        if stats['detection_precision'] + stats['detection_recall'] > 0:
            stats['detection_f1'] = (2 * stats['detection_precision'] * stats['detection_recall']) / \
    ...
    def update_plantation_data(self, comparison_report: Dict, 
        ...
        comp_by_plant_id = {c['plantation_id']: c for c in comparisons 
                           if c['status'] == 'matched'}
        comp_by_ml_id = {c['ml_tree_id']: c for c in comparisons 
                        if c['status'] == 'matched'}
        ...
        if 'trees' in updated_data:
            for tree in updated_data['trees']:
                ...
                if tree_id in comp_by_plant_id:
                    ...
                    ml_tree = next((t for t in ml_result.trees 
                                  if t.id == comp['ml_tree_id']), None)
    ...
    def generate_health_metrics(self, ml_result: InferenceResult) -> Dict:
        ...
        if ml_result.trees:
            ...
            metrics['tree_health'] = {
                ...
                'detection_confidence': float(np.mean([t.confidence for t in ml_result.trees]))
            }
            ...
            for species in ['cacao', 'shade']:
                ...
                if species_trees:
                    ...
                    metrics['tree_health'][f'{species}_crown_health'] = float(health_score)
        ...
        metrics['canopy_health'] = {
            ...
            'canopy_coverage': float(ml_result.metrics.get('cacao_tree_coverage', 0) + 
                                    ml_result.metrics.get('shade_tree_coverage', 0)),
            'understory_coverage': float(ml_result.metrics.get('understory_coverage', 0)),
            'bare_soil_percentage': float(ml_result.metrics.get('bare_soil_coverage', 0))
        }
        
        # Assess canopy health
        optimal_canopy_density = 0.45  # 45% is optimal for cabruca
        canopy_health_score = 1.0 - abs(ml_result.canopy_density - optimal_canopy_density) / optimal_canopy_density
        ...
        if ml_result.trees:
            shade_ratio = (metrics['tree_health']['shade_trees'] / 
                          max(metrics['tree_health']['cacao_trees'], 1))
            ...
            metrics['plantation_health'] = {
                ...
                'status': 'healthy' if overall_health > 0.7 else 
                         'moderate' if overall_health > 0.4 else 'poor'
            }
    ...
    def create_enhanced_visualization(self, image_path: str, 
                                     ml_result: InferenceResult,
                                     plantation_data: Dict) -> Dict:
        """
        ...
        if 'trees' in plantation_data:
            for tree in plantation_data['trees']:
                ...
                if len(coord) >= 2:
                    ...
                    circle = Circle((coord[0], coord[1]), 5, 
        ...
        for comp in comparison['comparisons']:
            if comp['status'] == 'matched':
                ...
                if ml_tree:
                    ...
                    circle = Circle((cx, cy), ml_tree.crown_diameter/2, 
        ...
        plt.suptitle('Theobroma Digital - ML Integration Analysis', fontsize=16, fontweight='bold')
        plt.tight_layout()
        
        # Save visualization
        output_path = Path(image_path).parent / f'{Path(image_path).stem}_integration_analysis.png'
...
class PlantationDataUpdater:
    ...
    @staticmethod
    def export_enhanced_geojson(plantation_data: Dict, ml_result: InferenceResult,
                               output_path: str):
        """
        ...
        for tree in plantation_data.get('trees', []):
            ...
            if tree.get('ml_coordinates') and tree.get('position_error', float('inf')) > 1.0:
...
def integrate_with_theobroma(model_path: str, plantation_data_path: str,
    ...
    with open(comparison_path, 'w') as f:
        json.dump({k: v for k, v in comparison.items() if k != 'ml_result'}, 
                 f, indent=2)
    ...
    print(f"Plantation Health: {health_metrics['plantation_health'].get('status', 'N/A').upper()}")
    print(f"Overall Score: {health_metrics['plantation_health'].get('overall_score', 0):.2%}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--plantation-data', type=str, required=True,
                       help='Path to plantation-data.json')
    parser.add_argument('--image', type=str, required=True,
                       help='Path to plantation image')
    parser.add_argument('--output', type=str, default='integration_results',
                       help='Output directory')
    
    args = parser.parse_args()
    
    integrate_with_theobroma(
        args.model,
        args.plantation_data,
        args.image,
        args.output
    )