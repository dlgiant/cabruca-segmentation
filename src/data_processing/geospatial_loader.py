...
class GeoJSONLoader:
    ...
    def to_planet_aoi(self, feature_index: int = 0) -> Dict:
        ...
        return {"type": "Polygon", "coordinates": list(aoi.geometry.exterior.coords)}
...
class SatelliteImageryLoader:
    ...
    def load_local_imagery(
        self, image_path: Union[str, Path], aoi: Optional[AOIBounds] = None
    ) -> Dict[str, np.ndarray]:
        """
    ...
    def search_planet_imagery(
        ...
        if response.status_code != 200:
            raise Exception(
                f"Planet API error: {response.status_code} - {response.text}"
            )

        return response.json().get("features", [])

    def download_planet_image(
        self,
        item_id: str,
        ...
        if asset_type not in assets:
            ...
            raise ValueError(
                f"Asset type {asset_type} not available. Available: {available}"
            )
...
class MultiBandProcessor:
    ...
    @staticmethod
    def stack_bands(
        bands: Dict[str, np.ndarray], band_order: Optional[List[str]] = None
    ) -> np.ndarray:
        """
    ...
    @staticmethod
    def resample_bands(
        bands: Dict[str, np.ndarray], target_shape: Tuple[int, int]
    ) -> Dict[str, np.ndarray]:
        """
...
class GeospatialDataPipeline:
    ...
    def process_local_imagery_for_aoi(
        self,
    ...
    def prepare_training_data(
        ...
        if normalize and stack:
            ...
            data = {
                k: self.band_processor.normalize_bands(
                    v[..., np.newaxis], method="percentile"
                )[..., 0]
