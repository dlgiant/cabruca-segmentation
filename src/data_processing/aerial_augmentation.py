...
class GeometricAugmentations:
    ...
    @staticmethod
    def rotate_nadir(
        image: np.ndarray, angle: float, mask: Optional[np.ndarray] = None
        ...
        if mask is not None:
            rotated_mask = cv2.warpAffine(
                mask, M, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_REFLECT
            )
    ...
    def perspective_transform(
        image: np.ndarray, strength: float = 0.001, mask: Optional[np.ndarray] = None
    ) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        """
    ...
    @staticmethod
    def orthorectification_distortion(
        image: np.ndarray,
        ...
        elevation = elevation_variation * (
            np.sin(X * 0.5) * np.cos(Y * 0.3) + np.sin(X * 0.2) * np.sin(Y * 0.7) * 0.5
        )
...
class RadiometricAugmentations:
    ...
    def simulate_shadows(
        ...
        area_range: Tuple[float, float] = (0.1, 0.3),
    ) -> np.ndarray:
        """
    ...
    def histogram_matching(
        image: np.ndarray, reference_histogram: Optional[np.ndarray] = None
    ) -> np.ndarray:
        """
...
class AtmosphericAugmentations:
    ...
    @staticmethod
    def add_clouds(
        image: np.ndarray, coverage: float = 0.3, opacity: float = 0.7
    ) -> np.ndarray:
        """
        ...
        cloud_mask = (cloud_mask - cloud_mask.min()) / (
            cloud_mask.max() - cloud_mask.min()
        )
    ...
    def atmospheric_scattering(
        image: np.ndarray, wavelength_shift: float = 0.1
    ) -> np.ndarray:
        """
...
class SensorAugmentations:
    ...
    def add_sensor_noise(
        image: np.ndarray, noise_type: str = "gaussian", variance: float = 0.01
    ) -> np.ndarray:
        """
        Add sensor noise
    ...
    @staticmethod
    def motion_blur(
        image: np.ndarray, angle: float = 0, length: int = 10
    ) -> np.ndarray:
        """
    ...
    @staticmethod
    def chromatic_aberration(image: np.ndarray, shift: float = 2.0) -> np.ndarray:
        """
        ...
        for i, (dx, dy) in enumerate(shifts):
            if i < image.shape[2]:
                ...
                result[:, :, i] = cv2.warpAffine(
                    image[:, :, i], M, (w, h), borderMode=cv2.BORDER_REFLECT
                )
    ...
    @staticmethod
    def dead_pixels(image: np.ndarray, probability: float = 0.0001) -> np.ndarray:
        """
...
class TemporalAugmentations:
    ...
    @staticmethod
    def seasonal_color_shift(image: np.ndarray, season: str = "summer") -> np.ndarray:
        """
    ...
    @staticmethod
    def sun_angle_illumination(
        image: np.ndarray, sun_elevation: float = 45, sun_azimuth: float = 180
    ) -> np.ndarray:
        """
...
class MultispectralAugmentations:
    ...
    @staticmethod
    def band_misalignment(image: np.ndarray, max_shift: float = 5) -> np.ndarray:
        """
...
class AerialAugmentationPipeline:
    ...
    def _apply_orthorectification(self, image: np.ndarray, mask: Optional[np.ndarray]):
        ...
        image, mask = self.geometric.orthorectification_distortion(
            image, elevation, mask
        )
    ...
    def _apply_elastic(self, image: np.ndarray, mask: Optional[np.ndarray]):
        ...
        return (
            image,
            mask,
            {"alpha": self.config.elastic_alpha, "sigma": self.config.elastic_sigma},
        )
    ...
    def _apply_absorption(self, image: np.ndarray, mask: Optional[np.ndarray]):
        ...
        image = self.multispectral.atmospheric_absorption(
            image, absorption_bands, strength
        )
        return image, mask, {"bands": absorption_bands, "strength": strength}

    def get_random_augmentation_sequence(
        self, num_augmentations: int = 3
    ) -> List[Callable]:
        """
        ...
        selected = random.sample(
            all_augmentations, min(num_augmentations, len(all_augmentations))
        )
...
def create_preset_config(preset: str) -> AugmentationConfig:
    ...
    presets = {
        "light": AugmentationConfig(
            enabled_types=[AugmentationType.GEOMETRIC, AugmentationType.RADIOMETRIC],
            probability=0.3,
        ...
        "geometric_only": AugmentationConfig(
            enabled_types=[AugmentationType.GEOMETRIC], probability=0.8
        ),
        "atmospheric_only": AugmentationConfig(
            enabled_types=[AugmentationType.ATMOSPHERIC], probability=0.8
        ),
