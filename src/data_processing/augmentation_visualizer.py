...
class AugmentationVisualizer:
    ...
    def visualize_augmentation_grid(
        self,
        original: np.ndarray,
        ...
        for row, (aug_type, samples) in enumerate(augmentation_types.items()):
            ...
            ax.set_ylabel(
                aug_type, fontweight="bold", rotation=0, ha="right", va="center"
            )
            ax.axis("off")
    ...
    def visualize_histogram_comparison(
        ...
        if len(original.shape) == 3:
            ...
            for i, color in enumerate(colors):
                if i < original.shape[2]:
                    ...
                    axes[0, 1].plot(
                        hist_orig, color=color, alpha=0.7, label=color.capitalize()
                    )

                    # Augmented histogram
                    hist_aug = cv2.calcHist([augmented], [i], None, [256], [0, 256])
                    axes[1, 1].plot(
                        hist_aug, color=color, alpha=0.7, label=color.capitalize()
                    )
        else:
        ...
        plt.suptitle(
            "Histogram and Statistics Comparison", fontsize=16, fontweight="bold"
        )
        plt.tight_layout()
    ...
    def create_augmentation_report(
        ...
        if save_dir:
            plt.savefig(
                save_dir / "augmentation_report.png", dpi=150, bbox_inches="tight"
            )

        plt.show()

    def _create_overlay(
        self, image: np.ndarray, mask: np.ndarray, alpha: float
    ) -> np.ndarray:
        """
        ...
        overlay = cv2.addWeighted(
            image.astype(np.uint8), 1 - alpha, mask_colored.astype(np.uint8), alpha, 0
        )
