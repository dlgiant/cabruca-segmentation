...
class InferenceResult:
    ...
    def summary(self) -> Dict:
        ...
        return {
            ...
            'avg_crown_diameter': np.mean([t.crown_diameter for t in self.trees]) if self.trees else 0,
...
class BatchInferenceEngine:
    ...
    def _remove_duplicate_instances(self, instances: List[Dict], 
    ...
    def _generate_batch_report(self, results: List[InferenceResult], 
                              output_dir: Path):
        """Generate comprehensive batch processing report."""
        # Aggregate statistics
        total_trees = sum(len(r.trees) for r in results)
        total_cacao = sum(sum(1 for t in r.trees if t.species == 'cacao') for r in results)
        total_shade = sum(sum(1 for t in r.trees if t.species == 'shade') for r in results)
        ...
        print(f"Total trees detected: {total_trees} ({total_cacao} cacao, {total_shade} shade)")
...
class VisualizationTools:
    ...
    @staticmethod
    def create_overlay(image: np.ndarray, result: InferenceResult, 
        ...
        overlay[mask] = (overlay[mask] * (1 - alpha) + semantic_colored[mask] * alpha).astype(np.uint8)
    ...
    @staticmethod
    def create_heatmap(result: InferenceResult, 
                       heatmap_type: str = 'density') -> np.ndarray:
        """
    ...
    @staticmethod
    def create_comparison_figure(image: np.ndarray, result: InferenceResult, 
        ...
        plt.suptitle(f"Cabruca Segmentation Analysis - {Path(result.image_path).name}", 
...
class ReportGenerator:
    ...
    @staticmethod
    def generate_tree_inventory(results: List[InferenceResult], 
    ...
    @staticmethod
    def export_to_geojson(result: InferenceResult, output_path: str,
                         crs: str = 'EPSG:4326'):
        """
    ...
    @staticmethod
    def generate_analysis_report(results: List[InferenceResult], 
                                output_dir: str):
        """
        ...
        inventory_df = ReportGenerator.generate_tree_inventory(results, str(inventory_path))
        ...
        axes[0, 0].pie(species_counts.values, labels=species_counts.index, 
                      autopct='%1.1f%%', colors=['green', 'brown'])
        ...
        # Confidence scores
        axes[1, 0].boxplot([inventory_df[inventory_df['species'] == s]['confidence'].values 
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Batch Inference for Cabruca Segmentation')
    ...
    parser.add_argument('--export-geojson', action='store_true',
                       help='Export results to GeoJSON')
