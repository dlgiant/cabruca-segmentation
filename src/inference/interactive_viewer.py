...
from inference.batch_inference import (
    BatchInferenceEngine, InferenceResult, 
...
class InteractiveViewer:
    ...
    def render_sidebar(self):
        ...
        if st.session_state.all_results:
            ...
            result_names = [Path(r.image_path).name for r in st.session_state.all_results]
    ...
    def render_heatmaps(self, result: InferenceResult):
        ...
        with col1:
            if heatmap_type == "Tree Density":
                ...
                st.image(heatmap, use_column_width=True, caption="Crown Diameter Heatmap")
            
            elif heatmap_type == "Species Distribution":
                heatmap = VisualizationTools.create_heatmap(result, 'species')
                st.image(heatmap, use_column_width=True, caption="Species Distribution (Green: Cacao, Red: Shade)")
    ...
    def render_metrics(self, result: InferenceResult):
        ...
        with col1:
            ...
            metrics_dict = {
                ...
                'Avg Confidence': f"{np.mean([t.confidence for t in result.trees]):.2f}" if result.trees else "N/A"
            }
    ...
    def export_geojson(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.geojson', delete=False) as tmp:
            ReportGenerator.export_to_geojson(
