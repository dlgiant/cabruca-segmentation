...
class CabrucaInference:
    ...
    def __init__(
        self, model_path: str, config: Optional[Dict] = None, device: str = None
    ):
        """
    ...
    def _load_model(
        self, model_path: str, config: Optional[Dict]
    ...
    def _predict_tiled(self, image_path: str) -> Dict:
        ...
        with rasterio.open(image_path) as src:
            ...
            for i in tqdm(
                range(0, len(tiles), self.batch_size), desc="Processing tiles"
            ):
                ...
                for j, (x, y, window) in enumerate(batch_tiles):
                    ...
                    tiles_results.append(
                        {"x": x, "y": y, "window": window, "outputs": tile_outputs}
                    )
    ...
    def _merge_tile_results(
        self, tiles_results: List[Dict], image_shape: Tuple[int, int]
    ) -> Dict:
        ...
        for tile_result in tiles_results:
            ...
            if "semantic" in outputs:
                ...
                semantic_map[y : y + window.height, x : x + window.width] = (
                    semantic_pred
                )
        ...
        if instance_masks:
            ...
            unique_instances = {"masks": [], "boxes": [], "labels": []}
        ...
        if "instances" in outputs:
            ...
            if isinstance(instances, list) and len(instances) > 0:
                ...
                results["instances"] = {
                    ...
                    "labels": instance_data.get("labels", torch.tensor([]))
                    .cpu()
                    .numpy(),
                    "scores": instance_data.get("scores", torch.tensor([]))
                    .cpu()
                    .numpy(),
    ...
    def export_to_geojson(
        self, results: Dict, image_path: str, output_path: str, crs: str = "EPSG:4326"
    ):
        """
                            ...
                            if len(coords) >= 3:
                                ...
                                feature = {
                                    ...
                                    "properties": {
                                        "id": i,
                                        "class": (
                                            "cacao_tree" if label == 1 else "shade_tree"
                                        ),
...
def run_inference(
    image_path: str, model_path: str, output_dir: str, config: Dict = None
):
    """
