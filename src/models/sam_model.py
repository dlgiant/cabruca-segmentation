...
class TreePromptEngineering:
    ...
    def generate_point_prompts(self, image: np.ndarray, 
                              tree_type: str = 'cacao') -> List[Tuple[int, int]]:
        """
    ...
    def generate_box_prompts(self, image: np.ndarray,
                           detections: Optional[List] = None) -> List[List[int]]:
        """
    ...
    def _is_tree_candidate(self, image: np.ndarray, box: List[int],
                          tree_type: str = 'cacao') -> bool:
        ...
        if len(region.shape) == 3:
            ...
            if tree_type == 'cacao':
                ...
                return vegetation_score > 30  # Higher threshold for brighter shade trees
...
class SAMTreeSegmenter:
    ...
    def load_model(self, checkpoint_path: Optional[str] = None):
        ...
        try:
            ...
            self.logger.error("segment-anything not installed. Please install it first.")
            raise
    ...
    def segment_with_boxes(self, image: np.ndarray,
    ...
    def _classify_single_tree(self, image: np.ndarray, mask: np.ndarray,
                             box: List[int]) -> str:
    ...
    def _classify_by_features(self, masked_image: np.ndarray, 
                             mask: np.ndarray) -> str:
    ...
    def fine_tune(self, train_data: Dict[str, Any], 
