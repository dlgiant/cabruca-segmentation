...
class ExperimentTracker:
    ...
    def __init__(self, config: Dict, experiment_name: str = None):
        ...
        self.config = config
        self.use_mlflow = (
            config.get("tracking", {}).get("mlflow", False) and MLFLOW_AVAILABLE
        )
        self.use_wandb = (
            config.get("tracking", {}).get("wandb", False) and WANDB_AVAILABLE
        )
    ...
    def _init_mlflow(self):
        ...
        if self.use_mlflow:
            mlflow.set_tracking_uri(
                self.config.get("tracking", {}).get("mlflow_uri", "file:./mlruns")
            )
    ...
    def _init_wandb(self):
        ...
        if self.use_wandb:
            wandb.init(
                project=self.config.get("tracking", {}).get(
    ...
    def _init_tensorboard(self):
        ...
        if self.use_tensorboard:
            log_dir = (
                Path(self.config.get("output_dir", "outputs"))
...
class MemoryEfficientTrainer:
    ...
    def __init__(self, config: Dict):
        ...
        self.best_metric = (
            float("inf")
            if config["training"].get("minimize_metric", True)
            else -float("inf")
        )
        self.best_epoch = 0
        ...
        # Checkpointing
        self.checkpoint_dir = (
            Path(config["output_dir"]) / self.tracker.experiment_name / "checkpoints"
        )
    ...
    def _log_system_info(self, device: torch.device):
        ...
        self.tracker.log_metrics(
            {f"system/{k}": v for k, v in info.items() if isinstance(v, (int, float))}
        )
    ...
    def _get_param_groups(self, group_config: List[Dict]) -> List[Dict]:
        ...
        for group in group_config:
            if group["name"] == "backbone":
                ...
                params = list(self.model.instance_head.roi_heads.parameters()) + list(
                    self.model.semantic_head.classifier.parameters()
                )
            elif group["name"] == "custom":
                params = list(self.model.crown_estimator.parameters()) + list(
                    self.model.density_estimator.parameters()
                )
            else:
                continue
    ...
    def _create_scheduler(self) -> Optional[object]:
        ...
        if sched_type == "cosine":
            ...
            scheduler = ExponentialLR(
                self.optimizer, gamma=sched_config.get("gamma", 0.95)
            )
        else:
            scheduler = None
    ...
    def _calculate_agroforestry_metrics(
    ...
    def train(self):
        ...
        early_stopping_patience = self.config["training"].get(
        ...
        for epoch in range(self.current_epoch, num_epochs):
            ...
            monitor_metric = self.config["training"].get(
                "monitor_metric", "val/total_loss"
            )
            current_metric = val_metrics.get(
                monitor_metric, val_metrics.get("val/total_loss", 0)
            )
        ...
        print(
            f"Best {self.config['training'].get('monitor_metric', 'val/total_loss')}: {self.best_metric:.4f}"
        )
    ...
    def _print_epoch_summary(self, epoch: int, metrics: Dict, epoch_time: float):
        ...
        # Patience status
        print(
            f"  Patience: {self.patience_counter}/{self.config['training'].get('early_stopping_patience', 20)}"
        )
    ...
    def _save_training_history(self):
        ...
        for key in history:
            history[key] = [
                float(v) if isinstance(v, torch.Tensor) else v for v in history[key]
            ]
