...
class TrainingMonitor:
    ...
    def _load_config(self) -> Dict:
        ...
        config_files = list(self.experiment_dir.glob('*.yaml')) + \
    ...
    def create_loss_plot(self, history: Dict) -> go.Figure:
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=('Total Loss', 'Instance Loss', 'Semantic Loss', 'Crown & Density Loss'),
        ...
        if 'train/total_loss' in history:
            fig.add_trace(
                go.Scatter(x=epochs, y=history['train/total_loss'],
                          name='Train', line=dict(color='blue')),
                row=1, col=1
            )
        if 'val/total_loss' in history:
            fig.add_trace(
                go.Scatter(x=epochs, y=history['val/total_loss'],
        ...
        if 'train/instance_loss' in history:
            fig.add_trace(
                go.Scatter(x=epochs, y=history['train/instance_loss'],
                          name='Train Instance', line=dict(color='blue', dash='dash')),
                row=1, col=2
            )
        if 'val/instance_loss' in history:
            fig.add_trace(
                go.Scatter(x=epochs, y=history['val/instance_loss'],
                          name='Val Instance', line=dict(color='red', dash='dash')),
                row=1, col=2
            )
        ...
        if 'train/semantic_loss' in history:
            fig.add_trace(
                go.Scatter(x=epochs, y=history['train/semantic_loss'],
                          name='Train Semantic', line=dict(color='green')),
                row=2, col=1
            )
        if 'val/semantic_loss' in history:
            fig.add_trace(
                go.Scatter(x=epochs, y=history['val/semantic_loss'],
                          name='Val Semantic', line=dict(color='orange')),
                row=2, col=1
            )
        ...
        if 'train/crown_loss' in history:
            fig.add_trace(
                go.Scatter(x=epochs, y=history['train/crown_loss'],
                          name='Crown', line=dict(color='purple')),
                row=2, col=2
            )
        if 'train/density_loss' in history:
            fig.add_trace(
                go.Scatter(x=epochs, y=history['train/density_loss'],
                          name='Density', line=dict(color='brown')),
                row=2, col=2
            )
    ...
    def create_metrics_plot(self, history: Dict) -> go.Figure:
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=('mAP@0.5', 'Tree Count Accuracy', 'Canopy IoU', 'Shade Distribution Score'),
            vertical_spacing=0.1,
            horizontal_spacing=0.1
        )
        ...
        if 'val/mAP_50' in history:
            fig.add_trace(
                go.Scatter(x=epochs, y=history['val/mAP_50'],
                          name='mAP@0.5', line=dict(color='blue')),
                row=1, col=1
            )
        ...
        if 'val/total_count_accuracy' in history:
            fig.add_trace(
                go.Scatter(x=epochs, y=history['val/total_count_accuracy'],
                          name='Count Accuracy', line=dict(color='green')),
                row=1, col=2
            )
        ...
        if 'val/canopy_iou' in history:
            fig.add_trace(
                go.Scatter(x=epochs, y=history['val/canopy_iou'],
                          name='Canopy IoU', line=dict(color='orange')),
                row=2, col=1
            )
        ...
        if 'val/shade_distribution_score' in history:
            fig.add_trace(
                go.Scatter(x=epochs, y=history['val/shade_distribution_score'],
                          name='Shade Score', line=dict(color='purple')),
                row=2, col=2
            )
        ...
        fig.update_layout(height=600, showlegend=True,
                         title_text="Agroforestry Metrics")
    ...
    def create_learning_rate_plot(self, history: Dict) -> go.Figure:
        ...
        fig = go.Figure()
        fig.add_trace(
            go.Scatter(x=epochs, y=history['train/learning_rate'],
    ...
    def create_system_gauges(self, metrics: Dict) -> go.Figure:
        ...
        fig.add_trace(
            go.Indicator(
                ...
                title={'text': f"Memory % ({metrics.get('memory_used_gb', 0):.1f}/{metrics.get('memory_total_gb', 0):.1f} GB)"},
                gauge={'axis': {'range': [0, 100]},
                      'bar': {'color': "darkgreen"},
                      'steps': [
...
def main():
    ...
    with tab1:
        ...
        if history:
            ...
            # Learning rate
            st.plotly_chart(monitor.create_learning_rate_plot(history), use_container_width=True)
        else:
    ...
    with tab2:
        ...
        if history:
            # Metrics plot
            st.plotly_chart(monitor.create_metrics_plot(history), use_container_width=True)
    ...
    with tab3:
        ...
        # System gauges
        st.plotly_chart(monitor.create_system_gauges(system_metrics), use_container_width=True)
        ...
        with col2:
            ...
            if 'gpu_name' in system_metrics:
                st.json({
                    ...
                    'Temperature (°C)': system_metrics.get('gpu_temperature', 'N/A')
                })
        ...
        if checkpoint_info:
            ...
            with col2:
                if st.button("🔄 Resume Training"):
                    ...
                    st.code(f"python train.py --config config.yaml --resume {checkpoint_info['path']}")
    ...
    with tab5:
        ...
        if monitor.tensorboard_dir.exists():
            ...
            if st.button("🚀 Launch TensorBoard"):
                subprocess.Popen(['tensorboard', '--logdir', str(monitor.tensorboard_dir)])
