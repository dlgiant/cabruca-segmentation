...
import json
from scipy.spatial import distance_matrix, Voronoi, voronoi_plot_2d
...
class AgroforestryMetrics:
    ...
    def calculate_tree_count_accuracy(self, predictions: Dict, 
        ...
        metrics['cacao_count_accuracy'] = 1.0 - abs(pred_cacao - gt_cacao) / max(gt_cacao, 1)
        metrics['shade_count_accuracy'] = 1.0 - abs(pred_shade - gt_shade) / max(gt_shade, 1)
        metrics['total_count_accuracy'] = 1.0 - abs(pred_total - gt_total) / max(gt_total, 1)
        ...
        if len(pred_labels) > 0 and len(gt_labels) > 0:
            ...
            if matched_labels:
                ...
                metrics['species_classification_accuracy'] = correct_species / len(matched_labels)
            else:
        ...
        for species_id, species_name in [(1, 'cacao'), (2, 'shade')]:
            tp = sum(1 for p, g in matched_labels if p == species_id and g == species_id)
            fp = sum(1 for p, g in matched_labels if p == species_id and g != species_id)
            fn = sum(1 for p, g in matched_labels if p != species_id and g == species_id)
    ...
    def calculate_canopy_coverage(self, predictions: Dict, 
                                 ground_truth: Dict) -> Dict:
        """
        ...
        if pred_semantic is not None and gt_semantic is not None:
            ...
            metrics['canopy_coverage_relative_error'] = abs(pred_coverage - gt_coverage) / max(gt_coverage, 0.01)
            ...
            metrics['cacao_coverage_accuracy'] = 1.0 - abs(pred_cacao_coverage - gt_cacao_coverage)
            metrics['shade_coverage_accuracy'] = 1.0 - abs(pred_shade_coverage - gt_shade_coverage)
        ...
        if 'crown_map' in predictions:
            ...
            metrics['total_crown_area'] = np.sum(crown_map > 0) * 0.25  # Assuming 0.5m pixel resolution
    ...
    def calculate_spacing_uniformity(self, predictions: Dict) -> Dict:
        ...
        metrics['cv_nearest_neighbor'] = std_nn_distance / avg_nn_distance if avg_nn_distance > 0 else 0
        ...
        clark_evans_r = avg_nn_distance / expected_distance if expected_distance > 0 else 0
        ...
        if len(tree_positions) >= 4:
            try:
                ...
                if cell_areas:
                    ...
                    metrics['voronoi_area_cv'] = np.std(cell_areas) / np.mean(cell_areas)
            except:
                pass
        
        return metrics
    
    def analyze_shade_distribution(self, predictions: Dict, 
                                  ground_truth: Dict) -> Dict:
        """
        ...
        shade_cacao_ratio = len(shade_positions) / len(cacao_positions) if len(cacao_positions) > 0 else 0
        ...
        metrics['shade_ratio_optimality'] = 1.0 - abs(shade_cacao_ratio - optimal_ratio) / optimal_ratio
    ...
    def calculate_instance_metrics(self, predictions: Dict, 
                                  ground_truth: Dict) -> Dict:
        """
        ...
        # Detection metrics
        matched = self._match_predictions_to_gt(pred_boxes, gt_boxes, self.iou_threshold)
        tp = len(matched)
    ...
    def calculate_crown_iou(self, predictions: Dict, ground_truth: Dict) -> Dict:
        ...
        if pred_areas and gt_areas:
            ...
            metrics['crown_area_accuracy'] = 1.0 - abs(pred_mean_area - gt_mean_area) / gt_mean_area
        
        return metrics
    
    def compare_with_plantation_data(self, predictions: Dict, 
                                    plantation_file: str) -> Dict:
        """
            ...
            for feature in plantation_data['features']:
                if 'geometry' in feature:
                    ...
                    if coords:
                        ...
                        species = feature.get('properties', {}).get('species', 'unknown')
        ...
        if pred_positions and plantation_trees:
            ...
            metrics['plantation_detection_rate'] = total_matched / total_plantation if total_plantation > 0 else 0
            metrics['plantation_precision'] = total_matched / total_predicted if total_predicted > 0 else 0
            metrics['plantation_recall'] = total_matched / total_plantation if total_plantation > 0 else 0
            
            # F1 score
            if metrics['plantation_precision'] + metrics['plantation_recall'] > 0:
                metrics['plantation_f1'] = (2 * metrics['plantation_precision'] * metrics['plantation_recall']) / \
        ...
        if 'instances' in predictions and plantation_species:
            ...
            if matched_trees and pred_labels is not None:
                ...
                for pred_idx, plant_idx, _ in matched_trees:
                    if pred_idx < len(pred_labels) and plant_idx < len(plantation_species):
                        pred_species = pred_labels[pred_idx]
                        true_species = species_mapping.get(plantation_species[plant_idx].lower(), 0)
                        if pred_species == true_species:
                            correct_species += 1
                
                metrics['plantation_species_accuracy'] = correct_species / len(matched_trees)
    ...
    def _match_instances(self, predictions: Dict, ground_truth: Dict) -> List[Tuple[int, int]]:
    ...
    def _calculate_box_iou_matrix(self, boxes1: np.ndarray, boxes2: np.ndarray) -> np.ndarray:
    ...
    def _calculate_iou_matrix(self, masks1: np.ndarray, masks2: np.ndarray) -> np.ndarray:
    ...
    def _calculate_ripleys_k(self, points: np.ndarray, radii: List[float] = None) -> Dict:
    ...
    def _calculate_shade_effectiveness(self, shade_positions: List, 
                                      cacao_positions: List) -> Dict:
    ...
    def _match_trees_by_distance(self, pred_positions: List, 
                                 gt_positions: List, 
    ...
    def _calculate_ap_at_iou(self, pred_boxes, pred_labels, pred_scores,
    ...
    def _calculate_class_ap(self, pred_boxes, pred_labels, pred_scores,
                           gt_boxes, gt_labels, class_id):
...
def visualize_evaluation_results(metrics: Dict, save_path: str = None):
    ...
    ax3.pie(coverage_data.values(), labels=coverage_data.keys(), autopct='%1.1f%%',
    ...
    ax4.barh(list(spacing_metrics.keys()), list(spacing_metrics.values()),
    ...
    ax7.bar(shade_data.keys(), shade_data.values(), color=['darkgreen', 'lightgreen', 'olive'])
