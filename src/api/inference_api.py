...
class ComparisonRequest(BaseModel):
    ...
    distance_threshold: float = Field(2.0, description="Maximum distance for tree matching (meters)")

class BatchInferenceRequest(BaseModel):
    """Request for batch processing."""
    image_paths: List[str]
    output_format: str = Field("json", description="Output format: json, geojson, excel")
...
async def startup_event():
    ...
    try:
        ...
        if os.path.exists(PLANTATION_DATA_PATH):
            integration_module = TheobromaIntegration(
                MODEL_PATH,
...
async def compare_with_plantation(
    ...
    try:
        ...
        if plantation_data:
            ...
            integration = TheobromaIntegration(
                MODEL_PATH,
                str(plantation_path)
            )
        else:
        ...
        health_metrics = integration.generate_health_metrics(
        ...
        response = {
            ...
            "health_report": {
                "overall_score": health_metrics['plantation_health'].get('overall_score', 0),
...
async def get_visualization(job_id: str):
    ...
    if not viz_path.exists():
        ...
        if image_path and os.path.exists(image_path):
            ...
            VisualizationTools.create_comparison_figure(
                image, result, str(viz_path)
            )
