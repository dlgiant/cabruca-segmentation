...
API_URL = os.getenv(
    "API_URL", "http://cabruca-stg-alb-428619257.sa-east-1.elb.amazonaws.com"
)
...
# Header
st.markdown(
    '<h1 class="main-header">🌳 Cabruca Segmentation Dashboard</h1>',
    unsafe_allow_html=True,
)
...
with st.sidebar:
    st.image(
        "https://via.placeholder.com/300x100/4CAF50/FFFFFF?text=Cabruca+AI",
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.info(
                f"Model: {'Loaded' if health_data.get('model_loaded') else 'Mock Mode'}"
            )
        else:
            st.error("❌ API Error")
    except:
...
tab1, tab2, tab3, tab4 = st.tabs(
    ["🖼️ Image Segmentation", "📈 Analytics", "🗺️ Plantation Map", "⚙️ Settings"]
)
...
with tab2:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            fig1.update_layout(
                title="Trees Detected Over Time",
                xaxis_title="Date",
                yaxis_title="Count",
            )
        ...
        if response.status_code == 200:
            ...
            if "sample_data" in plantation_data:
                ...
                with col3:
                    st.metric(
                        "Avg Tree Density",
                        f"{data.get('average_tree_density', 0)} trees/ha",
                    )
            ...
            st.info(
                "📍 This is sample data. Upload actual GeoJSON files for real plantation mapping."
            )
...
with tab4:
    ...
    with col2:
        ...
        export_format = st.selectbox(
            "Export Format", ["GeoJSON", "Shapefile", "KML", "CSV"]
        )
