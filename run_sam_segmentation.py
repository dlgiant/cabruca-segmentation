...
def segment_image_with_points(
    segmenter: SAMTreeSegmenter, image: np.ndarray, interactive: bool = False
) -> Dict[str, Any]:
    """
    ...
    if interactive:
        ...
        points = segmenter.prompt_engineer.generate_point_prompts(
            image, tree_type="cacao"
        )
        points.extend(
            segmenter.prompt_engineer.generate_point_prompts(image, tree_type="shade")
        )
...
def segment_image_with_boxes(
    segmenter: SAMTreeSegmenter, image: np.ndarray
) -> Dict[str, Any]:
    """
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["automatic", "points", "boxes"],
        default="automatic",
        help="Segmentation mode",
    )
    parser.add_argument(
        "--interactive", action="store_true", help="Enable interactive point selection"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="configs/sam_config.json",
    ...
    config = SAMConfig(model_type=model_type, checkpoint_path=checkpoint_path)
    ...
    if args.image:
        ...
        # Process directory
        process_directory(
            segmenter, Path(args.input_dir), Path(args.output_dir), mode=args.mode
        )
