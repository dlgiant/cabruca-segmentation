...
def create_synthetic_aerial_image(width: int = 512, height: int = 512, 
...
def test_individual_augmentations(image: np.ndarray, 
    ...
    sun_results = [
        (sun_low, "Low Sun (20°)"),
        (sun_high, "High Sun (70°)")
    ]
    visualizer.visualize_augmentation_sequence(image, sun_results)


def test_pipeline_with_presets(image: np.ndarray, 
                              mask: Optional[np.ndarray] = None) -> None:
    """
    ...
    presets = ['light', 'moderate', 'heavy', 'geometric_only', 
               'atmospheric_only', 'sensor_artifacts']
    ...
    for preset_name in presets:
        ...
        # Apply augmentation
        aug_image, aug_mask, params = pipeline.augment(
            image, mask, return_params=True
        )
...
def test_custom_pipeline(image: np.ndarray, 
                        mask: Optional[np.ndarray] = None) -> None:
    """
    ...
    if mask is not None:
        ...
        visualizer.visualize_mask_augmentation(
...
def test_multispectral_augmentations(width: int = 256, height: int = 256) -> None:
    ...
    absorption = multi_aug.atmospheric_absorption(
        multispectral, [2, 4], 0.3
    )
